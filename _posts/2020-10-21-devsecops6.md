...
- **SonarQube**   <a name="list2_5"></a>
    ...
    Sonar 자체로도 좋지만 PMD / Findbugs / Checkstyle을 별도로 사용하고 Jenkins CI에서 가시적으로 볼 수 있도록해주는 Analysis Collector 플러그인을 사용할 수도 있습니다. 이것의 장점은 PMD / Findbugs / Checkstyle 구성을 SCM에 체크인하고 별도의 Sonar 서버에 의존하지 않고 Maven 빌드에 통합 할 수 있다는 것 입니다.

    오래된 라이브러리(예 : Java 8의 경우)를 수정하거나 업그레이드 혹은 대기하는 데 시간을 소비하게 되기 때문에 SonarQube는 현재 PMD, Checkstyle 및 Findbug를 폐기하고 자체 기술을 사용하여 [SonarJava](https://redirect.sonarsource.com/plugins/java.html)를 분석 및 개발하는 중 입니다. 또한 [SonarLint](http://www.sonarlint.org/index.html) 라는 개인 IDE 용 새 플러그인 세트도 있습니다.
...
- **Anchore**   <a name="list2_6"></a>
    ...
    엔진이 처음 시작되면 CVE 취약성 데이터를 포함하여 피드 데이터의 전체 동기화를 수행합니다. 이 첫 번째 동기화는 몇 시간 동안 지속될 수 있으며 이 시간 동안 Anchore Engine은 이미지를 분석 할 수 있지만 피드 동기화가 성공적으로 완료 될 때까지 정책 평가 또는 CVE 보고를 수행하지 않습니다.

    Anchore Engine은 REST API를 노출하지만 Anchore Engine과 상호 작용하는 가장 쉬운 방법은 Python PiP를 사용하여 설치할 수있는 Anchore CLI를 사용하는 것입니다.
